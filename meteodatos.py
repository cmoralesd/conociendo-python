...
def filtrar_dia(datos, dia):
    ...
    filas_filtradas = [fila for fila in datos if dia in fila]
    respuesta = [datos[0]] + filas_filtradas
...
def estadisticas_dia(datos, dia):
    ...
    # ---------------------------------------------
    cabe_clima = []
    cabe_clima = filtrar_dia(datos,dia)
    respuesta = filtrar_cabecera(cabe_clima, 'ts')
    respuesta.remove('ts')
    tmin = min(respuesta)
    tmax = max(respuesta)
    tmedia = round(sum(respuesta)/len(respuesta),1)
